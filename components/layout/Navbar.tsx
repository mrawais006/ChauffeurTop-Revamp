...
const servicesList = [
    ...
    { name: "Conference & Events", href: "/services/conference-events" },
    { name: "Luxury Tours", href: "/services/luxury-tours" },
    { name: "Cruise Transfers", href: "/services/cruise-ship-transfers" },
    { name: "Student Transfers", href: "/services/student-transfers" },
    { name: "Wedding Limos", href: "/services/wedding-limos" },
...
export function Navbar() {
    ...
    return (
        <nav
            className={cn(
                "fixed top-0 left-0 w-full z-[10001] transition-all duration-300 ease-in-out border-b bg-luxury-black/95 backdrop-blur-md",
                scrolled
                    ? "border-white/10 py-3 shadow-lg"
                    : "border-transparent py-4"
            )}
        >
            <div className="container mx-auto px-6 flex items-center justify-between">
                {/* Logo */}
                <Link href="/" className="relative z-[10000] group">
                    <Image
                        src="/logo/logo-1.png"
                        alt="Chauffeur Top"
                        width={200}
                ...
                <div className="hidden md:flex items-center space-x-6">
                    ...
                    <nav className="hidden md:flex space-x-6 items-center">
                        {navLinks.map((link) => (
                            ...
                            link.hasDropdown ? (
                                <div key={link.name} className="relative group">
                                    ...
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-0 w-64 bg-luxury-black/95 backdrop-blur-xl border-2 border-white/20 rounded-md shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 pt-2 pb-2">
                                        {/* Little arrow indicator */}
                                        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1 w-2 h-2 bg-luxury-black border-t border-l border-white/20 rotate-45"></div>
                                        ...
                                        <div className="flex flex-col">
                                            {servicesList.map((service) => (
                                                <Link
                                                    key={service.name}
                                                    href={service.href}
                                                    className="px-6 py-3 text-white/80 hover:text-luxury-gold hover:bg-white/5 transition-all text-base font-medium border-b border-white/10 last:border-0"
                                                >
                                                    {service.name}
                                                </Link>
                                            ))}
                        ...
                        <Link href="/booking">
                            <Button variant="gold" size="sm" className="bg-luxury-gold text-black hover:bg-white hover:text-luxury-black ml-4 font-bold rounded-md min-h-[44px] px-6">
                                Get Quote
                            </Button>
                        </Link>
                    </nav>
            ...
            {/* Mobile Sticky Bottom Bar - Single Get Quote Button */}
            {mounted && !mobileMenuOpen && createPortal(
                <div className="md:hidden fixed bottom-0 left-0 w-full p-3 bg-luxury-black/95 backdrop-blur-lg border-t-2 border-white/20 z-[100]">
                    <Link href="/booking" className="block">
                        <Button
                            variant="gold"
                            size="lg"
                            className="w-full bg-luxury-gold text-black hover:bg-white hover:text-luxury-black font-bold text-lg shadow-lg rounded-md min-h-[48px]"
                        >
                            Get Quote
                        </Button>
                    </Link>
